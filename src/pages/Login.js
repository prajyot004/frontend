...
const Login = () => {
  ...
  const handleOnSubmit = async (e) => {
    ...
    if (dataApi.success) {
      ...
      cookies.set("token", dataApi.token, { path: "/" });
        ...
        <div className="p-5 w-full bg-[#F2EAD3] border border-black max-w-md mx-auto rounded h-full">
          ...
          <form className="pt-6 flex flex-col gap-2 " onSubmit={handleOnSubmit}>
            ...
            <div className="grid">
              ...
              <Link
                to={"/forgot-password"}
                className="mt-4 block w-fit ml-auto hover:underline hover:text-[#3F2305]"
              >
                Forgot password ?
              </Link>
            </div>
            <button
              className="bg-[#6F4E37] border-2 border-black text-white hover:bg-[#3F2305] px-6 py-2 w-full max-w-[150px] rounded-full
          ...
          <p className="py-5">
            ...
            <Link
              to={"/sign-up"}
              className="text-black hover:text-[#3F2305] hover:underline"
            >
              Sign up
            </Link>
          </p>
